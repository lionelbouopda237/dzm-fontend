      ...
      {visible && (
        <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-[#040812]">
          ...
          <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.7, ease: 'easeOut' }} className="relative z-10 text-center">
            <motion.div initial={{ filter: 'blur(12px)' }} animate={{ filter: 'blur(0px)' }} transition={{ duration: 1.1 }} className="w-64 h-64 mx-auto rounded-[36px] dzm-card flex items-center justify-center p-6 shadow-[0_0_120px_rgba(70,164,255,0.18)]">
              <img src={dzmLogo} alt="DZM" className="max-h-52 object-contain" />
            </motion.div>
